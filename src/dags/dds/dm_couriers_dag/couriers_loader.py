...
class CouriersDestRepository:
    def insert_courier(self, conn: Connection, courier_data: dict) -> None:
        # Метод для вставки или обновления курьеров в базе данных
        with conn.cursor() as cur:
            cur.execute(
                ...
                """,
                {
                    "id": courier_data["_id"],  # Получаем id курьера
                    "name": courier_data["name"],  # Получаем имя курьера
                }
            )

class CouriersLoader:
    WF_KEY = "dm_couriers_load_workflow"  # Ключ для рабочего процесса загрузки курьеров
    LAST_LOADED_ID_KEY = "last_loaded_id"  # Ключ для хранения последнего загруженного id
    BATCH_LIMIT = 1000  # Лимит на количество загружаемых курьеров за раз

    def __init__(self, pg_dest: PgConnect, log: Logger) -> None:
        # Инициализация класса с соединением к PostgreSQL и логгером
        self.pg_dest = pg_dest
        self.dest = CouriersDestRepository()  # Создаем экземпляр репозитория курьеров
        self.log = log  # Устанавливаем логгер

    def load_couriers(self) -> None:
        # Метод для загрузки курьеров из источника данных
        with self.pg_dest.client() as conn:  # Используем клиент для соединения с БД
            wf_setting = self.get_last_loaded_id(conn)  # Получаем последнее загруженное значение id
            last_loaded_id = wf_setting[self.LAST_LOADED_ID_KEY]  # Извлекаем последнее загруженное значение id

            couriers_to_load = self.get_couriers_from_stg(last_loaded_id)  # Получаем курьеров из STG
            if not couriers_to_load:
                self.log.info("No new couriers found.")  # Логируем отсутствие новых курьеров
                return
